...
interface IPriceQuote {
  stockSymbol: string;
  lastPrice: number;
  ibrahim?: number; // Optional property for additional data
}
...
@Component({
  ...
  template: `
    <div class="bg-primary text-light p-3">
      <h3>
        Parent Component Received: {{ stockSymbol }} --
...
@Component({
  ...
  template: `
    <div class="dataflow-container bg-info">
      ...
      <h3 class="bg-info">
        Recevied : {{ company }} -
        {{ ibrahim | currency : 'USD' : 'symbol' : '1.2-2' }}
      </h3>
      <p>
        This Price component is responsible for managing the data flow in the
        application.
      </p>
    </div>
  `,
  ...
  constructor() {
    ...
    window.setInterval(() => {
      let priceQuote: IPriceQuote = {
        ...
        ibrahim: Math.random() * 100,
      };
      this.price = priceQuote.lastPrice;
      this.ibrahim = priceQuote.ibrahim || 0; // Optional property handling
...
@Component({
  ...
  template: `
    <div class="dataflow-container bg-warning">
      ...
      <h3 class="bg-warning">
        Recevied : {{ info.stockSymbol }} -
        {{ info.lastPrice | currency : 'USD' : 'symbol' : '1.2-2' }}
      </h3>

      <p>
        This notifier component is responsible for managing the data flow in the
        application.
      </p>
    </div>
  `,
